...
def build_plot_data(dice_result, iou_result, threshold_result):
...
def plot_results(plot_data, output_path):
    ...
    axes[0].set_ylim(0.719, 0.755)
    axes[0].set_yticks(np.arange(0.72, 0.751, 0.01))
    axes[1].set_ylim(0.569, 0.605)
    axes[1].set_yticks(np.arange(0.57, 0.601, 0.01))
    axes[2].set_ylim(0.595, 0.705)
    axes[2].set_yticks(np.arange(0.60, 0.701, 0.02))
    legend_kwargs = {
        "title": "Optimizer",
        "fontsize": 13,
        "title_fontsize": 13,
    ...
    axes[2].legend(loc="center right", **legend_kwargs)
...
def main(refit=False):
    ...
    csv_path = (SCRIPT_DIR / "../tmp/data/sim-1-2.csv").resolve()
    figure_path = PROJECT_ROOT / "figures" / "sim-1-2.png"
    figure_path.parent.mkdir(parents=True, exist_ok=True)
    csv_path.parent.mkdir(parents=True, exist_ok=True)

    print(f"Running simulation on {DEVICE}")

    metadata = cache_metadata("sim-1-2", {"sample_size": sample_size, "dimensions": dimensions})
    if not refit and cache_matches(csv_path, metadata):
        print(f"Loading existing CSV from {csv_path}")
        plot_data = read_plot_data_csv(csv_path)
        plot_results(plot_data, figure_path)
        print(f"Saved figure to {figure_path}")
        return

    if refit:
        print("refit=True, rerunning simulation even if CSV already exists")
    else:
    ...
    plot_data = build_plot_data(dice_result, iou_result, threshold_result)
