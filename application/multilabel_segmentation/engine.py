...
from .metrics import masked_index_bce_with_logits
...
def _run_epoch(model, loader, optimizer, device, scaler, amp_enabled, training: bool) -> Dict[str, float]:
    ...
    return {"loss": round(running_loss / max(len(loader.dataset), 1), 6)}


def train_model(
    model_name, dataset_name, data_root, output_dir, image_size, batch_size, epochs, lr, weight_decay,
    num_workers, device, max_train_samples=None, max_val_samples=None,
    ...
    for epoch in range(start_epoch, int(epochs) + 1):
        train_metrics = _run_epoch(model, train_loader, optimizer, device, scaler, amp_enabled, True)
        val_metrics = _run_epoch(model, val_loader, optimizer, device, scaler, amp_enabled, False)
...
def load_trained_model(checkpoint_path, device):
    ...
    model = build_model(checkpoint["model_name"], int(checkpoint["num_classes"])).to(device)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()
